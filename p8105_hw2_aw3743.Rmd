...
```{r}
# Read Mr. Trash Wheel data with proper handling of comma-separated numbers
mr_trash_df = read_excel("./data/202409 Trash Wheel Collection Data.xlsx", 
                        sheet = "Mr. Trash Wheel",
...
mr_trash_clean = mr_trash_df |>
  ...
  mutate(
    across(c(plastic_bottles, polystyrene, cigarette_butts, glass_bottles, 
           ...
           ~ as.numeric(gsub(",", "", .))),  # Remove commas and convert to numeric
    homes_powered = as.numeric(homes_powered),
    sports_balls = as.integer(round(sports_balls)),  # Round and convert to integer
    trash_wheel = "Mr. Trash Wheel"  # Add identifier
  ) |>
  select(-contains("note"))  # Remove note columns
